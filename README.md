...
| Skin    | Sky                                                   | Copy                                  |
| ------- | ----------------------------------------------------- | ------------------------------------- |
| `night` | stars, nebula, shooting stars, the moon — **default** | floating on the sky, lit by the moon  |
| `slate` | grid, wash, vignette                                  | panelled, flat, desaturated accent    |
...
`:root`, and that block **is** the night skin — so the served HTML paints the
default before Angular boots, and `@property` fallbacks agree with it.
`.scene.is-slate` and `.scene.is-day` in `app.css` are pure diffs against it;
`.scene.is-night` has no block of its own.
...
Nothing on screen says the toggle switches anything, so it carries a nudge
(`themeToggle.hint` in `content/app-shell*.json`) that says where the wheel is;
it goes away for good the first time the reader picks a skin.

`App` also has a self-guided tour that switches skin every 30s in the toggle's
order until the reader takes over. It is **switched off** at
`THEME_ROTATION_ENABLED` in `app.ts` — the mechanism is kept behind the flag so
turning it back on is a one-line change. It never runs under
